...
private:
  ...
  // Flag whether this array should deep copy using constructor in copy constructor
  bool _should_construct = false;
  // Host data pointer
...
public:
  ...
  unsigned int use_count() { return _counter ? *_counter : 0; }
  ...
  void deepCopy(const GPUArrayBase<T, dimension> & array, bool construct = false);
  // Sets whether this array should deep copy using constructor in copy constructor
  void shouldConstruct(bool flag) { _should_construct = flag; }

public:
  // Scalar copy operator
  void operator=(const T & scalar);
  // Shallow copy operator
  void operator=(const GPUArrayBase<T, dimension> & array) { shallowCopy(array); }
#endif
};
...
{
  Kokkos::View<T *, TargetSpace, Kokkos::MemoryTraits<Kokkos::Unmanaged>> t(target, n);
  Kokkos::View<const T *, SourceSpace, Kokkos::MemoryTraits<Kokkos::Unmanaged>> s(source, n);
  Kokkos::deep_copy(t, s);
  Kokkos::fence();
}
...
GPUArrayBase<T, dimension>::shallowCopy(const GPUArrayBase<T, dimension> & array)
{
  ...
  _should_construct = array._should_construct;
...
template <typename T, unsigned int dimension>
void
GPUArrayBase<T, dimension>::deepCopy(const GPUArrayBase<T, dimension> & array, bool construct)
{
  if (construct && !array._is_host_alloc)
...
public:
  ...
  void operator=(const std::vector<T> & vector) { copyVector(vector); }
  void operator=(const std::set<T> & set) { copySet(set); }
