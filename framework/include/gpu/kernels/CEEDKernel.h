...
#define CEED_KERNEL(name, parent)                                                                  \
  ...
  public:                                                                                          \
    ...
    std::string getQFunctionPath() { return std::string(__FILE__) + ":" + #name + "_qf"; }
...
public:
    ...
    for (auto type : _assembly.getUniqueElemTypes())
      for (auto qrule : _assembly.getUniqueQuadratures())
      {
        ...
        CeedQFunctionCreateInterior(
            _ceed, 1, kernel->getQFunction(), kernel->getQFunctionPath().c_str(), &qf);
        ...
        for (auto var : getMooseVariableDependencies())
        {
          ...
          for (auto tag : tags)
          {
            ...
            CeedQFunctionAddInput(qf, name.c_str(), var->count(), CEED_EVAL_INTERP);
            CeedQFunctionAddInput(
                qf, (name + "_grad").c_str(), var->count() * _mesh.dimension(), CEED_EVAL_GRAD);
  ...
  {
    auto object = static_cast<Kernel &>(*this);
    auto & system = _systems[_var.sys().number()];

    CeedQFunctionContextSetData(_ctx, CEED_MEM_HOST, CEED_USE_POINTER, sizeof(Kernel), &object);
